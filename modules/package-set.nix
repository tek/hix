...
{name ? null, lib, config, ...}: let
  ...
  options = {
    ...
    overrides = lib.mkOption {
      ...
      This option is set by environments (see [](#envs)), but package sets modules can be used outside of environments,
    ...
    customize = util.maybeOption (types.functionTo util.types.haskellPackages) {
      ...
      Takes an argument set `{ pkgs, packages }` and should return the modified `packages`.
