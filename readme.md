...
```
nix run 'github:tek/hix?ref=0.5.3#bootstrap'
```

You can create a new project in the current directory:

```
nix run 'github:tek/hix?ref=0.5.3#new' -- --name 'project-name' --author 'Your Name'
```
...
```nix
{
  inputs.hix.url = "github:tek/hix?ref=0.5.3";
}
```
