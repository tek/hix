...
in {
  test = builtins.toFile "managed-nom-test" ''
    ...
    [35m[1m>>>[0m Build with multi-fail2-0.1.0 failed in [34mmulti-fail1[0m
    [35m[1m>>>[0m Building targets in [33mlatest[0m with multi-fail1-0.1.0...
    [35m[1m>>>[0m Build with multi-fail1-0.1.0 failed in [34mmulti-fail1[0m
    ...
    check 'nix run .#bump -- ${args} 2>&1' "$target1" 'First run with failing package has wrong output'

    mk_fail 1 'multi-fail'
    mk_success 2 'multi-fail'

    target2="[35m[1m>>>[0m Building targets in [33mlatest[0m with multi-fail2-0.1.0...
    [35m[1m>>>[0m Build with multi-fail2-0.1.0 failed in [34mmulti-fail2[0m
    [35m[1m>>>[0m Building targets in [33mlatest[0m with multi-fail1-0.1.0...
    [35m[1m>>>[0m Build with multi-fail1-0.1.0 failed in [34mmulti-fail2[0m
    ...
    check 'nix run .#bump -- ${args} 2>&1' "$target2" 'Second run with failing package has wrong output'
  '';
}
