{pkgs, testlib, ...}: let
  ...
  step ${pkgs.xh}/bin/xh get --follow --auth test:test $_hackage_url/package/local1-1.1.0/revision/1.cabal

  output_match 'semigroups >=0\.19 && <0\.21'
  step ${pkgs.xh}/bin/xh get --follow --auth test:test $_hackage_url/package/local2-1.2.0/revision/1.cabal
  '';
