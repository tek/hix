...
let
  ...
  libx = import ../default.nix { inherit lib; };
  ...
  unknownHackage = name: ''
  An override refers to the nonexistent Hackage server config ${colors.option name}.
  '';
  ...
  Internal error: An override refers to the Hackage server ${colors.option name}, but no config was passed to the API.
  '';
